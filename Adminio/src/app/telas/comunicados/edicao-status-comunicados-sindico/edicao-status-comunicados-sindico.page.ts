...
@Component({
  selector: 'app-edicao-status-comunicados-sindico',
  templateUrl: './edicao-status-comunicados-sindico.page.html',
  styleUrls: ['./edicao-status-comunicados-sindico.page.scss'],
})
export class EdicaoStatusComunicadosSindicoPage implements OnInit {
  ...
  async editstatus(comunicado_id){
      ...
      let dados = {
        ...
        this.provider.dadosApi(dados, 'api_adm.php').subscribe(async data => {
          ...
          if(data['consultpagepaut']) {
            ...
            return new Promise(resolve => {
              let dados = {
                  requisicao : 'editstatuscomunicado',
                  status : this.status,
