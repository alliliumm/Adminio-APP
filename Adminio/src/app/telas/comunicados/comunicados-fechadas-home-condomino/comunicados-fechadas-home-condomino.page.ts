...
@Component({
  selector: 'app-comunicados-fechadas-home-condomino',
  templateUrl: './comunicados-fechadas-home-condomino.page.html',
  styleUrls: ['./comunicados-fechadas-home-condomino.page.scss'],
})
export class ComunicadosFechadasHomeCondominoPage implements OnInit {
  assunto: string = "";
  conteudo: string = "";
  status: string = "";
  ...
  mostrarcomunicado(comunicado_id, assunto, conteudo, status, comunicados_sindico_id, data_enviado, hora_enviado, cont_sinalizacoes){
    this.router.navigate(['/mostrar-comunicados-fechadas-condomino/' + comunicado_id + '/' + assunto + '/' + conteudo + '/' + status + '/' + comunicados_sindico_id + '/' + data_enviado + '/' + hora_enviado + '/' + cont_sinalizacoes]);
  }
